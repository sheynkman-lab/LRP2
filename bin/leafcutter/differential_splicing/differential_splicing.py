...
def differential_splicing(counts, x, confounders = None, max_cluster_size=10, min_samples_per_intron=5, min_samples_per_group=4, min_coverage=0, min_unique_vals = 10, device = "cpu", **kwargs):
    ...
    for clu in cluster_ids:
        ...
        print(f'Sample totals shape: {sample_totals.shape}')
        print(f'Sample totals: {sample_totals}')
        samples_to_use=sample_totals >= 1.0 # handle Expected counts
        ...
        introns_to_use=(cluster_counts >= 1.0).sum(0)>=min_samples_per_intron # only look at introns used by at least 5 samples
        ...
        if x_subset.dtype.kind in 'OUS': # categorical x
            print(f"CATEGORICAL X_SUBSET LOGIC")
            ...
            print(f"CONTINUOUS X_SUBSET LOGIC")
        ...
        print(f"X SUBSET BEFORE THE CRASH: ")
        print(f"{x_subset.shape}")
        print(f"{x_subset}")
