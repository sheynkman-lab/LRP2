...
#GTF2GENEPRED_PROG = "/opt/sqanti3/utilities/gtfToGenePred"
GTF2GENEPRED_PROG = os.path.join(sqanti_path, "src", "utilities", "gtfToGenePred")
GFFREAD_PROG = "gffread"

# copied function from SQANTI 3.3 sqanti_qc.py
...
def extract_transcript_id(attr_str):
    ...
    tid_raw = m.group(1)
    if "PB." in tid_raw:
        pb = PB_REGEX.search(tid_raw)
        return pb.group(0) if pb else tid_raw
    return tid_raw

def process_gtf(infile, prefix):
    exon_coords = defaultdict(list)
    cds_coords = defaultdict(list)
    transcripts = {}  # tid -> (chrom, source, score, strand, frame, raw_attr)
    ...
    def write_output(outfile, feature_map, feature_name):
        with open(outfile, "w") as out:
            for tid, (chrom, source, score, strand, frame, attr_str) in transcripts.items():
                if tid not in feature_map:
                    continue
                starts, ends = zip(*feature_map[tid])
                tstart, tend = min(starts), max(ends)
                out.write(f"{chrom}\t{source}\ttranscript\t{tstart}\t{tend}\t{score}\t{strand}\t{frame}\t{attr_str}\n")
                for s, e in sorted(feature_map[tid]):
                    out.write(f"{chrom}\t{source}\t{feature_name}\t{s}\t{e}\t{score}\t{strand}\t{frame}\t{attr_str}\n")

    name_of_transcripts_file = f"{prefix}.transcript_exons_only.gtf"
    name_of_cds_file = f"{prefix}.cds_renamed_exon.gtf"
    write_output(name_of_transcripts_file, exon_coords, "exon")
    write_output(name_of_cds_file, cds_coords, "exon")

    print(f"[done] wrote {prefix}.cds_renamed_exon.gtf and {prefix}.transcript_exons_only.gtf")
    return name_of_transcripts_file, name_of_cds_file

## END: split GTF (i.e., rename CDS) embedded code
...
if __name__ == "__main__":
    ...
    FIELDNAMES = ['isoform_id', 'tx_cat', 'pr_splice_cat', 'tx_subcat', 'pr_splice_subcat',
