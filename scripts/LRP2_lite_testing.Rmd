...
orf_coords = read_tsv("../results/orf_calling/sample_best_orfs_mapped.tsv", show_col_types = FALSE)
...
# Extract sequences as named character vector
pb_seqs <- setNames(as.character(pb_fasta), names(pb_fasta))

# Translate each ORF to protein
orfs$protein_seq <- sapply(1:nrow(orfs), function(i) {
  pb_acc <- orfs$pb_acc[i]
  start <- orfs$orf_start[i]
  end <- orfs$orf_end[i]
  
  # Extract ORF region from transcript
  orf_seq <- substr(pb_seqs[[pb_acc]], start, end)
  
  # Translate to protein (stop at first stop codon)
  prot_seq <- as.character(translate(DNAString(orf_seq)))
  prot_seq <- sub("\\*.*", "", prot_seq)  # Remove stop codon and everything after
